          ...
          data: (profiles) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                final profileIndex = providerError != null ? index - 1 : index;
              ...
              separatorBuilder: (context, index) {
                final isAfterBanner = providerError != null && index == 0;
...
class _SavedLoginTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final providerId = dbRecord?.id;
    SummaryData? dbSummaryData;
    var dbSummaryLoading = false;
    SummaryData? portalSummaryData;
    var portalSummaryLoading = false;
    if (providerId != null) {
      final summaryAsync = ref.watch(
        dbSummaryProvider(DbSummaryArgs(providerId, record.kind)),
      );
      dbSummaryLoading = summaryAsync.isLoading;
      dbSummaryData = summaryAsync.when(
        data: (data) => data,
        loading: () => null,
        error: (error, stackTrace) => null,
      );
      final portalSummaryAsync = ref.watch(
        portalSummarySnapshotProvider(providerId),
      );
      portalSummaryLoading = portalSummaryAsync.isLoading;
      portalSummaryData = portalSummaryAsync.when(
        data: (data) => data,
        loading: () => null,
        error: (error, stackTrace) => null,
      );
    ...
    final mergedCounts = _mergeSummaryCounts(
      portalCounts: portalSummaryData?.counts,
      dbCounts: dbSummaryData?.counts,
    );
    final summaryWidget = _buildSummaryWidget(
      context: context,
      counts: mergedCounts,
      isLoading: dbSummaryLoading || portalSummaryLoading,
      hasProviderRecord: providerId != null,
    );
  ...
  String _buildSubtitle(BuildContext context, String host, DateTime? lastSync) {
  ...
  Widget? _buildSummaryWidget({
    ...
    required bool hasProviderRecord,
  }) {
    if (!hasProviderRecord) {
      return null;
    }
    final theme = Theme.of(context);
    final normalized = _normalizeAndOrderCounts(counts);
    if (normalized.isEmpty) {
      ...
      return Text(
        'Counts available after first sync.',
    ...
    final summaryText = normalized.entries
        .map((entry) => '${entry.key}: ${entry.value}')
        .join(' | ');
    return Text(summaryText, style: theme.textTheme.bodySmall);
  }

  Map<String, int> _mergeSummaryCounts({
    Map<String, int>? portalCounts,
    Map<String, int>? dbCounts,
  }) {
    if (portalCounts == null && dbCounts == null) {
      return const {};
    }
    final merged = <String, int>{};
    if (portalCounts != null) {
      merged.addAll(portalCounts);
    }
    if (dbCounts != null) {
      dbCounts.forEach((key, value) {
        if (value > 0) {
          merged[key] = value;
        }
      });
  ...
  Map<String, int> _normalizeAndOrderCounts(Map<String, int> counts) {
    if (counts.isEmpty) return const {};
    final normalized = <String, int>{};
    counts.forEach((key, value) {
      final normalizedKey = _normalizeCountLabel(key);
      normalized.update(
        normalizedKey,
        (previous) => value > previous ? value : previous,
        ifAbsent: () => value,
      );
    });
    final ordered = <String, int>{};
    const preferredOrder = ['Live', 'Movies', 'Series', 'Radio'];
    for (final label in preferredOrder) {
      if (normalized.containsKey(label)) {
        ordered[label] = normalized[label]!;
      }
    }
...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  void _trackImportProgress(int providerId) {
    ...
    _importProgressSubscription = service.watchProgress(providerId).listen((
      event,
    ) {
      if (!mounted) return;
  ...
  Widget? _buildImportProgressBanner() {
    ...
    final progress = _progressFraction(event);
  ...
  double _progressFraction(ProviderImportProgressEvent event) {
    switch (event.phase) {
      case 'started':
      case 'xtream.fetch':
      case 'm3u.fetch':
