...
class _FfmpegRestreamSession {
  ...
  Future<RestreamHandle?> start() async {
    ...
    return RestreamHandle(
      source: restreamedSource,
      dispose: dispose,
    );
  }
  ...
  Future<void> _startProcess(HttpRequest request) async {
    ...
    try {
      ...
      stderrSub = process.stderr
          ...
          .listen((line) => PlaybackLogger.videoInfo(
                'ffmpeg-restream',
                extra: {'stderr': line.trim()},
              ));
      unawaited(request.response.done.whenComplete(() async {
