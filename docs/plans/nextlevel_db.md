...
- [x] Per-category paging with caps/backoff plus radio coverage; global "*" paging only as a limited fallback. -> All Stalker modules (including Live/Radio) now stream per-category pages with tightened caps, short yields between requests, and throttled logging. The "*" fallback is capped to 25 pages and only used when categories fail, eliminating the minutes-long scroll and multi-novel logs.
