...
final providerFavoritesProvider = StreamProvider.autoDispose
    .family<List<ChannelWithFlags>, int>((ref, providerId) {
      final repository = ref.watch(channelRepositoryProvider);
      return repository.watchFavoriteChannels(providerId);
    });

final providerRecentPlaybackProvider = StreamProvider.autoDispose
    .family<List<RecentChannelPlayback>, int>((ref, providerId) {
      ...
      return historyDao.watchRecent(providerId: providerId, limit: 15).asyncMap(
        (histories) async {
          ...
          final joined =
              await (db.select(
                channels,
              )..where((tbl) => tbl.id.isIn(ids))).join([
...
class RecentChannelPlayback {
  RecentChannelPlayback({required this.history, this.channel, this.flags});
