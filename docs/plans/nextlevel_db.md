...
Use this backlog to track the “ultimate” database roadmap. Each checkbox should become an issue or milestone item.
...
- [ ] Model `providers`, `channel_groups`, `channels`, `stream_endpoints`, `epg_events` in Drift.
- [ ] Add mandatory indexes: `(provider_id, stable_key)` unique, `(group_id, sort_key)`, NOCASE `name`, EPG covering indexes, `stream_endpoints(channel_id, priority)`.
- [ ] Implement chunked bulk upserts (1–5 k rows) using `INSERT … ON CONFLICT DO UPDATE`.
- [ ] Build key-set pagination reads (no OFFSET) with prepared statements and off-isolate write queue.
- [ ] Apply WAL-friendly PRAGMAs (`journal_mode=WAL`, `synchronous=NORMAL`, cache sizing, `foreign_keys=ON`).
...
- [ ] Track tombstones for removed channels; schedule background hard-prune.
...
- [ ] Add FTS5 external-content tables for channels (name, alt names, group title) and optional EPG snippets using unicode61 tokenizer.
- [ ] Create triggers to keep FTS mirrors in sync on insert/update/delete.
- [ ] Build hot caches: LRU “recent channels” table + tiny in-memory buffer for last 3–5 logos/mini-EPG entries.
...
- [ ] Enforce EPG retention window (D‑2 … D+14) with nightly/idle sweeps (10 k-row chunks).
- [ ] Store per-channel `min(start_ts)`/`max(end_ts)` to short-circuit range queries.
- [ ] Run VACUUM only after large deletes and while idle/charging; expose IndexedDB size on web builds.
...
- [ ] Ship semantic migrations with DAO checksum tracking.
...
- [ ] Keep Android/iOS/macOS/Windows/Linux on sqlite3/FFI with identical schema + DAOs.
...
- [ ] Prototype adaptive prefetcher that predicts and preloads likely next channels.

## Implementation Snapshot
- [ ] DB bootstrap: schema + indices + PRAGMAs + DAO skeletons.
- [ ] Ingest engine: isolate parsing, chunked upserts, idempotent runs.
- [ ] Search: channel FTS + triggers.
- [ ] Retention: sweeper + compaction guardrails.
- [ ] Hot caches: recents + mini-EPG prewarm with key-set pagination.
- [ ] Migrations: integrity checks + downgrade coverage.
- [ ] Performance harness: scripted 10 k/100 k imports plus scroll/search benchmarks in CI.
