...
void main() {
  test('migrates schema from v1 to latest', () async {
    final tmpFile = File(
      '${Directory.systemTemp.path}/openiptv_migration_test.sqlite',
    );
    ...
    final legacy = OpenIptvDb.forTesting(executor, schemaVersionOverride: 1);
    await legacy.customSelect('SELECT 1').get();
    await legacy
        .into(legacy.providers)
        .insert(
    ...
    final importRunsExists = await migrated
        .customSelect(
    ...
    expect(pragma.single.data.values.first, OpenIptvDb.schemaVersionLatest);
