  ...
  group('StalkerPortalDiscovery', () {
    test(
      ...
      () async {
        ...
        final discovery = const StalkerPortalDiscovery();
    ...
    test('flips scheme to http when https candidate fails', () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      addTearDown(server.close);
      ...
      final probeSchemes = result.telemetry.probes
          .map((probe) => probe.uri.scheme)
          .toSet();
      expect(
    ...
    test('retries base-directory with fallback User-Agent after 403', () async {
      final server = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
      addTearDown(server.close);
